...
- `check`, `lint`, `impact`, and `linear` accept `--indent N` with `--json`, and the global
  `--no-color` option explicitly disables colored output, overriding an ambient `FORCE_COLOR`
  so it also strips styling from Typer-rendered help and usage errors (#20).
